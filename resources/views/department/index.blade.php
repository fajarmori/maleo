                    ...
                    <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 rounded-lg">
                        <table id="listDepartment" class="min-w-full divide-y divide-gray-300">
                            ...
                            <tr class="{{$department->deleted_at ? 'bg-red-200' : ''}}">
                                ...
                                <td class="whitespace-normal text-wrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">{{ $department->code }}</td>
                                <td class="whitespace-normal text-wrap px-3 py-4 text-sm text-gray-500">{{ $department->name }}</td>
                                @if(auth()->user()->type !== 2)
...
$(document).ready(function() {
    $('#listDepartment').DataTable({ ordering : false, dom: "<'sm:flex text-sm bg-gray-50 bg-gray-100/75'<'sm:basis-1/2 text-sm p-2'l><'sm:flex sm:basis-1/2 justify-end text-sm p-2'f>>"+'rtip', lengthMenu: [[10, 25, 50, -1], [10, 25, 50, "All"]], });
