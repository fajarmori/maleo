                    ...
                    <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 rounded-lg">
                        <table id="listOccupation" class="min-w-full divide-y divide-gray-300">
                            ...
                            <tr>
                                ...
                                <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">{{ $journey->name }}</td>
                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ $journey->name }}</td>
                                <td class="whitespace-nowrap flex px-3 py-4 text-sm text-gray-500">
                                    ...
                                    <form onsubmit="return confirm('Apakah anda yakin menghapus data {{$journey->name}} ?');" action="{{ route('journeys.destroy', $journey->id) }}" method="POST">
                                        @method('DELETE')
                                        @csrf
